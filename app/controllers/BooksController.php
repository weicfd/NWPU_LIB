...
class BooksController extends \BaseController {
	...
	public function index()
	{
		...
		$book_list = Books::select('book_id','title','author','isbn','description','category_id')
			->orderBy('book_id');
	...
	public function store()
	{
		...
		$createdID = "";
		$message = "";
		DB::transaction( function() use($books, &$createdID, &$message) {
			$db_flag = false;
			$user_id = Auth::id();
			$newId;
			$exist_book = Books::select('book_id','title','author','description','category_id')->where('isbn', '=', $books['isbn'])->get()->first();
			if($exist_book){
				if($exist_book['title']!=$books['title'] 
				|| $exist_book['author']!=$books['author'] 
				|| $exist_book['category_id']!=$books['category']){
					throw new Exception('Error! Same ISBN with different book infomation.');
				}
				$newId=$exist_book['book_id'];
				$message.=" Existing book. Located at " . $exist_book['description'];
			}else{
	...
	{
		$books = (array)json_decode(Input::all()['edit_book_data']);	
		DB::transaction( function() use($books, $id) {
			
			$book_title = Books::where('book_id',$id)
			->update([
			'title' => $books['title'],
			'author' => $books['author'],
			'isbn' => $books['isbn'],
			...
			$exist_book = Books::where('isbn', '=', $books['isbn'])->count();
			if($exist_book>1){
				throw new Exception('Error! Already have a book with same ISBN.');
			}
	...
	public function destroy($id)
	{
		$conditions = array(
		    ...
		    $book_borrowed = Issue::select()
		        	->where($conditions)
		        	->count();
			
		    if ($book_borrowed>0) {
			      throw new Exception('Error! There are some issues borrowed away yet.');
		    }
		
		DB::transaction( function() use ($id){
			Issue::where('book_id', '=', $id)->delete();
	...
	public function renderEditBooks() {
		$db_control = new HomeController();

		return View::make('panel.editbook')
            ->with('categories_list', $db_control->categories_list);
	}
