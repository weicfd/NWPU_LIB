...
class LogController extends \BaseController {
	...
	public function index()
	{
		...
		for($i=0; $i<count($logs); $i++){
			...
			$logs[$i]['return_at'] =  date('d-M', strtotime("+1 months", strtotime($logs[$i]['issued_at'])));
	...
	public function store()
	{
		...
		$sid = Student::select('student_id')->where('roll_num', '=', $studentID);
		$sid = $sid->get();

		if(count($sid) == 0){
			throw new Exception('Invalid Student ID');
		}else{
			$sid = $sid{0}['student_id'];
				...
				if($books_issued >= $max_allowed){
					...
					if($book == NULL){
						...
						if($book_availability != 1){
							...
							DB::transaction( function() use($bookID, $sid) {
								$log = new Logs;
